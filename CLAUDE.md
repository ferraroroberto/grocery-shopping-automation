...
[`docs/architecture.mmd`](docs/architecture.mmd) is a hand-authored Mermaid diagram of this repo's own internal structure (the FastAPI/PWA and legacy Streamlit front ends, the UI-free `src/` data layer, `automation/`'s Playwright cart automation, and the external `claude-local-calls`/`voice-transcriber` dependencies). Update it in the same PR as any material structural change (a new mode/route, a module moved, a new external dependency wired in) — same anti-staleness contract as this repo's own `.fleet.toml` `description` field. It is not auto-generated and not covered by the pytest suite.
